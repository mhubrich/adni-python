...
# Change both paths
directory = '/home/mhubrich/ADNI_intnorm_deepROI2_AD'
brain = np.load('/home/mhubrich/predicted_brain_AD_pn_threshold_005.npy')
...
for f in files:
    ...
    scan *= brain
    # Keep only important region (new dims: 80x80x84)
    scan = scan[5:85,:,:][:,11:91,:][:,:,0:84]
    # Pool down by 4x4x4 (new dims: 20x20x21)
    scan = block_reduce(scan, (4,4,4), func=np.mean, cval=0)
    # Pad with zeros (new dims: 21x21x21)
    scan = np.pad(scan, ((0,1), (0,1), (0,0)), 'constant', constant_values=0)
