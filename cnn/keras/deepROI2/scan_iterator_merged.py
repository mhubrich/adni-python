...
class ScanIterator(Iterator):
    def __init__(self, scans, image_data_generator,
        ...
        if self.load_all_scans:
            self.scansNC = np.zeros((self.nb_sample,) + (20, 20, 20), dtype='float32')
            self.scansAD = np.zeros((self.nb_sample,) + (21, 21, 21), dtype='float32')
        ...
        for c in classes:
            for scan in groups[c]:
                ...
                if self.load_all_scans:
                    ...
                    self.scansAD[i] = self.load_scan(scan.path.replace('deepROI2_NC', 'deepROI2_AD'))
                    #self.scans[i] = self.load_scan(scan.path.replace('deepROI2_NC', 'avgpool444'))
                else:
                    self.scansNC.append(scan.path)
                    self.scansAD.append(scan.path.replace('deepROI2_NC', 'deepROI2_AD'))
                    #self.scans.append(scan.path.replace('deepROI2_NC', 'avgpool444'))
                i += 1
    ...
    def next(self):
        ...
        batch_xNC = np.zeros((current_batch_size,) + (1, 20, 20, 20))
        batch_xAD = np.zeros((current_batch_size,) + (1, 21, 21, 21))
        ...
        if self.class_mode == 'sparse':
            ...
            return [batch_xNC, batch_xAD]
        return [batch_xNC, batch_xAD], batch_y
