...
class ScanIterator(Iterator):
    ...
    def rand_voxel(self, target_size, x=(10, 77), y=(10, 97), z=(10, 77)):
        """
    ...
    def next(self):
        ...
        diff = np.zeros((current_batch_size,) + (1,))
        ...
        for i, j in enumerate(index_array):
            ...
            if not isinstance(scan, np.ndarray):
                scan = self.load_scan(scan)
            diff[i] = (0.21848500141177771 - np.mean(scan) + 0.054772224340099124) / 0.10892483592033386
            x = self.expand_dims(x, self.dim_ordering)
            batch_x[i] = x
            positions[i] = [(voxel[0]-10)/(77.0+1-self.target_size[0]-10.0),
                            (voxel[1]-10)/(97.0+1-self.target_size[1]-10.0),
                            (voxel[2]-10)/(77.0+1-self.target_size[2]-10.0)]
        ...
        if self.class_mode == 'sparse':
            ...
            return [batch_x, positions, diff]
        return [batch_x, positions, diff], batch_y
