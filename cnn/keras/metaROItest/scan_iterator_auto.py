...
class ScanIterator(Iterator):
    ...
    def next(self):
        ...
        diff_1 = np.zeros((current_batch_size,) + (128,))
        diff_2 = np.zeros((current_batch_size,) + (128,))
        diff_3 = np.zeros((current_batch_size,) + (128,))
        diff_4 = np.zeros((current_batch_size,) + (128,))
        diff_5 = np.zeros((current_batch_size,) + (128,))
        ...
        for i, j in enumerate(index_array):
            ...
            diff_1[i] = np.load(self.scans[j].replace('metaROI1', 'metaROI1_auto'))
            diff_2[i] = np.load(self.scans[j].replace('metaROI1', 'metaROI2_auto'))
            diff_3[i] = np.load(self.scans[j].replace('metaROI1', 'metaROI3_auto'))
            diff_4[i] = np.load(self.scans[j].replace('metaROI1', 'metaROI4_auto'))
            diff_5[i] = np.load(self.scans[j].replace('metaROI1', 'metaROI5_auto'))
            if False:
        ...
        if self.class_mode == 'sparse':
            ...
            return [batch_x1, diff_1, batch_x2, diff_2, batch_x3, diff_3, batch_x4, diff_4, batch_x5, diff_5]
        #return [batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, diff], batch_y
        return [batch_x1, diff_1, batch_x2, diff_2, batch_x3, diff_3, batch_x4, diff_4, batch_x5, diff_5], batch_y
