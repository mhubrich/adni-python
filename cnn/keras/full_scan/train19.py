...
from cnn.keras.models.AVG444.model2 import build_model
...
path_checkpoints = '/home/mhubrich/checkpoints/adni/full_scan_19_CV' + fold
...
def train():
    ...
    scans_train = read_imageID(path_ADNI, '/home/mhubrich/ADNI_CV_MCI/' + fold + '_train')
    ...
    hist = model.fit(x=x_train,
                     ...
                     nb_epoch=num_epoch,
                     callbacks=cbks,
                     class_weight={0:max(len(g['MCI']), len(g['AD']))/float(len(g['MCI'])),
                                   1:max(len(g['MCI']), len(g['AD']))/float(len(g['AD']))},
