...
for k in filter_length:
    ...
    for i in range(len(predictions)):
        if predictions[i][1] != classes['AD']:
            continue
        if np.round(imageIDs[predictions[i][0]]) != 1:
            continue
...
np.save('importanceMap_1_35_fold_' + fold + '_AD_correct.npy', importanceMap2)

img = nib.Nifti1Image(importanceMap2, np.eye(4))
nib.save(img, 'importanceMap_1_35_fold_' + fold + '_AD_correct.nii')
