...
for k in filter_length:
    ...
    for i in range(len(predictions)):
        if predictions[i][1] != classes['Normal']:
            continue
        if np.round(imageIDs[predictions[i][0]]) != 0:
            continue
...
np.save('importanceMap_1_35_fold_' + fold + '_NC_correct.npy', importanceMap2)

img = nib.Nifti1Image(importanceMap2, np.eye(4))
nib.save(img, 'importanceMap_1_35_fold_' + fold + '_NC_correct.nii')
