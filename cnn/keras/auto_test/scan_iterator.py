...
class ScanIterator(Iterator):
    def __init__(self, scans, image_data_generator,
        ...
        if self.dim_ordering == 'tf':
            self.image_shape = self.target_size + (128,)
        else:
            self.image_shape = (128,) + self.target_size
        ...
        if self.load_all_scans:
            self.scans = np.zeros((self.nb_sample,) + (128, 21, 21, 21), dtype='float32')
        else:
            self.scans = []
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            x = self.get_scan(scan=self.scans[j])
