...
def build_model(input_shape=(1, 10, 12, 14)):
        name = 'AAL34'
        do = 0.1
	model = Sequential()
        model.add(GaussianNoise(0.001, input_shape=input_shape, name=name+'_noise1'))
	model.add(Convolution3D(32, 3, 5, 5, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv1'))
        model.add(Dropout(do, name=name+'_dropout1'))
	model.add(Convolution3D(32, 3, 3, 5, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv2'))
	...
	model.add(Convolution3D(128, 2, 2, 2, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv5'))
