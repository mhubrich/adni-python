...
def build_model(input_shape=(1, 8, 8, 8)):
	...
	model.add(Convolution3D(32, 3, 3, 3, activation='relu', border_mode='same', W_regularizer=l2(l)))
        model.add(Dropout(0.1))
	model.add(Convolution3D(64, 3, 3, 3, activation='relu', W_regularizer=l2(l)))
        model.add(Dropout(0.1))
	...
	model.add(Convolution3D(256, 3, 3, 3, activation='relu', W_regularizer=l2(l)))
        model.add(Dropout(0.1))
        model.add(Convolution3D(512, 2, 2, 2, activation='relu', W_regularizer=l2(l)))
        model.add(Dropout(0.1))
	model.add(Flatten())
