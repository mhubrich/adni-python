...
def build_model(input_shape=(1, 22, 22, 22)):
    ...
    x = Dense(512, activation='relu', W_regularizer=l2(0.0001), name=name+'_dense2')(x)
    x = Dropout(0.2, name=name+'_dropout9')(x)
    x = Dense(256, activation='relu', W_regularizer=l2(0.0001), name=name+'_dense3')(x)
    x = Dropout(0.2, name=name+'_dropout10')(x)
    x = Dense(256, activation='relu', W_regularizer=l2(0.0001), name=name+'_dense4')(x)
    x = Dropout(0.2, name=name+'_dropout11')(x)
    x = Dense(128, activation='relu', W_regularizer=l2(0.0001), name=name+'_dense5')(x)
    x = Dropout(0.2, name=name+'_dropout12')(x)
    x = Dense(128, activation='relu', W_regularizer=l2(0.0001), name=name+'_dense6')(x)
    x = Dropout(0.2, name=name+'_dropout13')(x)

    output = Dense(1, activation='sigmoid', W_regularizer=l2(0.0001), name=name+'_dense7')(x)
