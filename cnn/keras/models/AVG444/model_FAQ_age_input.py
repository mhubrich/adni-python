...
def build_model(input_shape=(1, 22, 22, 22)):
    name = 'AVG444'
    do = 1.0/14
    input1 = Input(shape=input_shape, name=name+'_input1')
    x = GaussianNoise(0.001, name=name+'_noise1')(input1)
    ...
    input2 = Input(shape=(1,), name=name+'_input2')
    y = GaussianNoise(0.05, name=name+'_noise2')(input2)

    input3 = Input(shape=(1,), name=name+'_input3')
    z = GaussianNoise(0.05, name=name+'_noise3')(input3)

    x = merge([x, y, z], mode='concat')
    ...
    return Model(input=[input1, input2, input3], output=output)
