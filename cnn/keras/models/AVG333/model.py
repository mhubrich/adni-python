...
def build_model(input_shape=(1, 30, 30, 30)):
    name = 'AVG333'
    do = 1.0/18
    ...
    x = Convolution3D(32, 7, 7, 7, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv2')(x)
    ...
    x = Convolution3D(64, 5, 5, 5, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv4')(x)
    ...
    x = Convolution3D(128, 3, 3, 3, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv6')(x)
    x = Dropout(do, name=name+'_dropout6')(x)
    x = Convolution3D(256, 3, 3, 3, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv7')(x)
    x = Dropout(do, name=name+'_dropout7')(x)
    x = Convolution3D(256, 3, 3, 3, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv8')(x)
    x = Dropout(do, name=name+'_dropout8')(x)
    x = Convolution3D(512, 2, 2, 2, activation='relu', W_regularizer=l2(0.0001), name=name+'_conv9')(x)
    ...
    x = Dropout(0.5, name=name+'_dropout12')(x)
