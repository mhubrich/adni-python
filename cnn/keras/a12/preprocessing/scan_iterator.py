    ...
    def __init__(self, scans, image_data_generator,
        ...
        for c in classes:
            for scan in groups[c]:
                ...
                self.scans[i] = s
                self.filenames.append(scan.path + '_' + scan.imageID + '_' + scan.group)
                if not self.shuffle:
                    self.voxels.append(_rand_voxel())
                i += 1
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            if self.shuffle:
                ...
                voxel = self.voxels[j]
            x = _load_scan(scan=self.scans[j], voxel=voxel, nb_slices=self.nb_slices, dim_ordering=self.dim_ordering)
