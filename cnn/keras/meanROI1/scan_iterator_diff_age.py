...
class ScanIterator(Iterator):
    def __init__(self, scans, image_data_generator,
        ...
        self.age = np.zeros((self.nb_sample, 1), dtype=np.float32)
        class_pos = [0]
        i = 0
        for c in classes:
            for scan in groups[c]:
                ...
                if self.load_all_scans:
                    ...
                    self.scans2[i] = self.load_scan(scan.path.replace('meanROI1_1', 'meanROI1_2'))
                    self.scans3[i] = self.load_scan(scan.path.replace('meanROI1_1', 'meanROI1_3'))
                    self.scans4[i] = self.load_scan(scan.path.replace('meanROI1_1', 'meanROI1_4'))
                    self.scans5[i] = self.load_scan(scan.path.replace('meanROI1_1', 'meanROI1_5'))
                else:
                    self.scans1.append(scan.path)
                    self.scans2.append(scan.path.replace('meanROI1_1', 'meanROI1_2'))
                    self.scans3.append(scan.path.replace('meanROI1_1', 'meanROI1_3'))
                    self.scans4.append(scan.path.replace('meanROI1_1', 'meanROI1_4'))
                    self.scans5.append(scan.path.replace('meanROI1_1', 'meanROI1_5'))
                self.diff[i] = np.load(scan.path.replace('meanROI1_1', 'meanROI1_diff'))
                self.age[i] = (scan.age - 55.31) / (95.8164 - 55.31)
                i += 1
            class_pos.append(i)
    ...
    def next(self):
        ...
        batch_age = np.zeros((current_batch_size, 1))
        ...
        if self.class_mode == 'sparse':
            ...
            return [batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, batch_diff, batch_age]
        return [batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, batch_diff, batch_age], batch_y
