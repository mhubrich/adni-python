        ...
        for c in classes:
            for scan in groups[c]:
                ...
                if self.load_all_scans:
                    ...
                    self.scans2[i] = self.load_scan(scan.path.replace('mean_1', 'mean_2'))
                    self.scans3[i] = self.load_scan(scan.path.replace('mean_1', 'mean_3'))
                    self.scans4[i] = self.load_scan(scan.path.replace('mean_1', 'mean_4'))
                    self.scans5[i] = self.load_scan(scan.path.replace('mean_1', 'mean_5'))
                else:
                    self.scans1.append(scan.path)
                    self.scans2.append(scan.path.replace('mean_1', 'mean_2'))
                    self.scans3.append(scan.path.replace('mean_1', 'mean_3'))
                    self.scans4.append(scan.path.replace('mean_1', 'mean_4'))
                    self.scans5.append(scan.path.replace('mean_1', 'mean_5'))
                self.diff[i] = np.load(scan.path.replace('mean_1', 'mean_diff'))
                i += 1
            class_pos.append(i)
