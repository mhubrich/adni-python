...
class ScanIterator(Iterator):
    def __init__(self, scans, image_data_generator,
        ...
        if self.load_all_scans:
            ...
            self.scans2 = np.zeros((self.nb_sample,) + (21, 21, 21), dtype='float32')
            self.scans3 = np.zeros((self.nb_sample,) + (14, 14, 14), dtype='float32')
            self.scans4 = np.zeros((self.nb_sample,) + (13, 13, 13), dtype='float32')
            self.scans5 = np.zeros((self.nb_sample,) + (22, 22, 22), dtype='float32')
        else:
            self.scans1 = []
            self.scans2 = []
            self.scans3 = []
        ...
        self.ages = np.zeros((self.nb_sample, 1), dtype=np.float32)
        i = 0
        for c in classes:
            for scan in groups[c]:
                ...
                if self.load_all_scans:
                    ...
                    self.scans2[i] = self.load_scan(scan.path.replace('AAL64', 'AAL65'))
                    self.scans3[i] = self.load_scan(scan.path.replace('AAL64', 'AAL34'))
                    self.scans4[i] = self.load_scan(scan.path.replace('AAL64', 'AAL35'))
                    self.scans5[i] = self.load_scan(scan.path.replace('AAL64', 'AAL61'))
                else:
                    self.scans1.append(scan.path)
                    self.scans2.append(scan.path.replace('AAL64', 'AAL65'))
                    self.scans3.append(scan.path.replace('AAL64', 'AAL34'))
                    self.scans4.append(scan.path.replace('AAL64', 'AAL35'))
                    self.scans5.append(scan.path.replace('AAL64', 'AAL61'))
                self.diff[i] = np.load(scan.path.replace('AAL64', 'AAL_diff'))
                self.ages[i] = (scan.age - 55.31) / (95.8164 - 55.31)
                i += 1
    ...
    def next(self):
        ...
        batch_x2 = np.zeros((current_batch_size,) + (1, 21, 21, 21))
        batch_x3 = np.zeros((current_batch_size,) + (1, 14, 14, 14))
        batch_x4 = np.zeros((current_batch_size,) + (1, 13, 13, 13))
        batch_x5 = np.zeros((current_batch_size,) + (1, 22, 22, 22))
        batch_diff = np.zeros((current_batch_size, 5))
        batch_ages = np.zeros((current_batch_size, 1))
        ...
        if self.class_mode == 'sparse':
            ...
            return [batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, batch_diff, batch_ages]
        return [batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, batch_diff, batch_ages], batch_y
