...
from cnn.keras.final_metaROI.model4 import build_model
...
path_checkpoints = '/home/mhubrich/checkpoints/adni/final_metaROI_single4_1_CV' + fold
...
def load_data(scans, flip=False):
    ...
    X = np.zeros((nb_samples, 1, 13, 13, 13), dtype=np.float32)
    i = 0
    for c in classes:
        for scan in groups[c]:
            s = nib.load(scan.path).get_data()
            y[i] = 0 if scan.group == classes[0] else 1
            X[i] = s[ranges[3][0],:,:][:,ranges[3][1],:][:,:,ranges[3][2]]
            i += 1
            if flip:
                X[i] = np.flipud(s)[ranges[3][0],:,:][:,ranges[3][1],:][:,:,ranges[3][2]]
                y[i] = y[i-1]
                i += 1
            del s
    return X, y
