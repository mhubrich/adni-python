...
def create_importanceMap(filter_length, fold, path):
    ...
    for k in filter_length:
        predictions = []
        imageIDs = {}
        with open(path + str(k) + '_' + fold + '_full_all.csv', 'rb') as csvfile:
        ...
        for i in range(len(predictions)):
            if predictions[i][0] not in importanceMap:
                importanceMap[predictions[i][0]] = [np.zeros(target_size, dtype=np.float32), np.zeros(target_size, dtype=np.float32)]
            ...
            y = predictions[i][3]
            z = predictions[i][4]
            importanceMap[predictions[i][0]][predictions[i][1]][x, y, z] += importance / count_voxel(pos=(x,y,z), n=k)

    for key in importanceMap:
        importanceMap[key][0] /= len(filter_length)
        importanceMap[key][1] /= len(filter_length)
