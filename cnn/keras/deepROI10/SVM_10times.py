...
path_predictions = 'predictions/predictions_deepROI10_3_fliter_'
...
def evaluate(a, y, y_pred, i, fold, c):
    ...
    a[i, fold, 0, c] = accuracy_score(y, np.round(y_pred))
    a[i, fold, 1, c] = (tn + tp) / 2.0
    a[i, fold, 2, c] = tn
    a[i, fold, 3, c] = tp


metrics_val = np.zeros((10, 10, 4, len(C)), dtype=np.float32)
metrics_test = np.zeros((10, 10, 4, len(C)), dtype=np.float32)

importanceMap_all = pickle.load(open('importanceMap_all_deepROI12_fliter_135_full', 'rb'))
#importanceMap_all = create_importanceMap([1, 3, 5], pre, path_predictions)
#importanceMap = np.load('importanceMap_135_cnn.npy')
...
for i in range(1, 11):
    for fold in range(1, 11):
        ...
        for c in range(len(C)):
                ...
                evaluate(metrics_val, y_val, y_pred, i-1, fold-1, c)

                y_pred = clf.predict(X_test)
                evaluate(metrics_test, y_test, y_pred, i-1, fold-1, c)


np.save('SVM/metrics_val_deepROI10_' + pre + '_000001_135_10times.npy', metrics_val)
np.save('SVM/metrics_test_deepROI10_' + pre + '_000001_135_10times.npy', metrics_test)
