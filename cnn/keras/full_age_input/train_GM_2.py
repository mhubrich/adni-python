...
from cnn.keras.models.AVG444.model4_age import build_model
...
path_checkpoints = '/home/mhubrich/checkpoints/adni/full_scan_age_GM_2_CV' + fold
...
def train():
    ...
    x_train, age_train, y_train = load_data(scans_train, flip=True)

    scans_val = read_imageID(path_ADNI, '/home/mhubrich/ADNI_CV_mean2/' + fold + '_val')
    x_val, age_val, y_val = load_data(scans_val, flip=False)
    ...
    cbks = [callbacks.print_history(),
            callbacks.flush(),
            Evaluation([x_val, age_val], y_val, batch_size,
    ...
    hist = model.fit(x=[x_train, age_train],
                     y=y_train,
                     nb_epoch=num_epoch,
                     callbacks=cbks,
